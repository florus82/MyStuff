...
modfolder = '/home/florus/MSc/RS_Data/MODIS/raw'
...
parc_XY    = getXYfromShape('/home/florus/MSc/GIS_Data/Plots/gasparri/parcelas_uniqID_reproj_unnamed.shp')
parc_Attri = getAttributesALL('/home/florus/MSc/GIS_Data/Plots/gasparri/parcelas_uniqID_reproj_unnamed.shp')
...
def MODext(modHDF, bands, cells, res):
    ...
    for count, scene in enumerate(modHDF):
        ...
        for i in range(len(cells['col'])):
            ...
            if qual in VI_oki:
                ...
                res['NBR'].append('NA')
            print(i)
        print('Scene {} of 702 processed ({} %)'.format(count+1, round((count+1) / 702,2) * 100))

    df    = pd.DataFrame(data = res)
    endtime = time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime())
    df.to_csv('/home/florus/MSc/ContiExtr/ContiBM' + str(endtime) + '.csv', sep=',',index=False)
