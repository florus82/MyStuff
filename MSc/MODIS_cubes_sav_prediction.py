...
for m in ModL:
    ...
    path2 = 'Y:/_students_data_exchange/FP_FP/RS_Data/MODIS/rasterized/'
    cubis = getFilelist(path + 'Modelling/GAP_FILLED/Single_VIs/prediction/' + tile + '/SP', '.sav')
    ...
    iter100 = pd.read_csv(path + 'Modelling/GAP_FILLED/Single_VIs/Modelling/runs_check100_final/All100Runs_continue.csv')
    savs    = getFilelist(path + 'Modelling/GAP_FILLED/Single_VIs/Modelling/runs_check100_final/sav_continue', '.sav')
    sav     = getMinMaxMedianSAV(iter100, savs)
    # print(sav[0][0])
    # print(sav[1][0])
    # print(sav[2][0])
    # sav_names = ['Min', 'Max', 'Median']
    sav = [['orig', joblib.load('Y:/_students_data_exchange/FP_FP/Seafile/myLibrary/MSc/Modelling/GAP_FILLED/Single_VIs/Modelling/runs_final_check/NDVI_GrowSeas_and_SeasPos_Mediannot_smoothSeasPar_NDVI.sav')]]
    sav_names = ['Initial']
    # iterate over the 3 different models
    for mod in range(len(sav_names)):
        ...
        #counti = 7
            out_ds = gtiff_driver.Create(path + 'Modelling/GAP_FILLED/Single_VIs/prediction/' + tile + '/check/' + tile + '_' + rasNames[counti] + '_' + sav_names[mod] + '.tif', til.shape[1],
